...
class Header extends React.Component {
  ...
  componentDidMount() {
    ...
    if (loggedUserJson) {
      ...
      if (loggedUser) this.setState({loggedUser: loggedUser});
    } else {
      var parent = this;
      axios.get(config.baseUrl.api+'logged')
                .then(function (response) {
                    localStorage.setItem('loggedUser', JSON.stringify(response.data));
                    parent.setState({loggedUser: response.data});
                });
    }
  }
          ...
          <Nav className="ml-md-0">
            ...
            >
              <DropdownToggle nav caret style={{ color: "#C1C3CF", padding: 0 }}>
                ...
                >
                  <img src={this.state.loggedUser.image} alt="..." />
                </span>
                <span className={`small d-sm-down-none ${s.accountCheck}`}>{this.state.loggedUser.fullName}</span>
